...
class ActionSpace:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.xyrot, torch.Tensor) or not isinstance(self.mask, torch.Tensor):
            raise TypeError("ActionSpace.xyrot and ActionSpace.mask must be torch.Tensor")
        if self.xyrot.ndim != 2 or int(self.xyrot.shape[-1]) != 3:
            raise ValueError(f"ActionSpace.xyrot must have shape [N,3], got {tuple(self.xyrot.shape)}")
        if self.mask.ndim != 1 or int(self.mask.shape[0]) != int(self.xyrot.shape[0]):
            raise ValueError(
                f"ActionSpace.mask must have shape [N], got {tuple(self.mask.shape)} for N={int(self.xyrot.shape[0])}"
            )
